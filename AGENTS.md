...
This repository contains the Minimal Blog theme for ScissorHands.NET, not the engine. The theme renders static HTML using Razor; browser interactions use plain JavaScript. Do not introduce a Blazor client runtime, backend, or UI framework unless requested.
...
The committed sample link at `themes/minimal-blog` must remain a relative symbolic link to `../../src`. Keep linked Razor sources included in the sample build while excluding their `bin` and `obj` directories. See the [preview instructions](sample/README.md).
...
- [Build, release, and deploy](.github/workflows/main.yaml) restores packages from NuGet.org and builds the Release solution for pushes to `main`, conventional type-prefixed branches, and `v*` tags, pull requests targeting `main`, and manual `workflow_dispatch` runs. New `v*` tag pushes package the theme, generate the sample site, create a GitHub release, and deploy the demo to GitHub Pages after the build succeeds. The test step is commented out until test projects are added.
